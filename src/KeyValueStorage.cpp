...
bool KeyValueStorage::getBuffer(const String &key, Buffer &value) const {
    // key does not exist
...
void KeyValueStorage::getSetBuffer(const String &key, Buffer &value, const Buffer &fallback) {
    // key does not exist
    if (mKeys.count(key) == 0)
